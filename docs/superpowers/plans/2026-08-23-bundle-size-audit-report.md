...
`@sonoscope/core` changed from 60,823 B gzip and 51,268 B brotli to 60,744 B gzip and 51,288 B brotli.
...
| Extract WebGL2 compile helpers | -76 B | +110 B | Kept | Pass |
| Terser passes `3` to `2` | +4 B | -53 B | Kept, within 50 B gzip limit | Pass |
| Enable `drop_console` and remove internal exports | 0 B | 0 B | Kept | Pass |
| Frequency-ruler Y-scale helper | +37 B | -15 B | Reverted | Pass |
| Total | -79 B | -20 B | Kept changes only | Pass |
...
| --- | ---: | ---: | ---: |
| Gzip | 60,823 B | 60,744 B | -79 B |
| Brotli | 51,268 B | 51,288 B | -20 B |
...
### WebGL2 compile extraction

`viewers/shared/webgl2-compile.ts` now owns shader creation, compilation, linking, and shader cleanup for spectrogram and waveform renderers. Its error-style parameter preserves each renderer's previous error messages, including spectrogram source line numbering.

`jscpd` reported the shader compilation duplicate at `spectrogram/renderers/webgl2-program.ts:131-137` and `waveform/renderers/webgl2-shaders.ts:41-47`. The extraction reduced gzip by 76 B and added 110 B brotli.
...
Changed Terser compression passes from three to two. With the final refactors in place, the measured gzip increase was 4 B and brotli decreased by 53 B, within the agreed 50 B gzip limit.
...
`npm test`: 58 test files and 513 tests passed.
