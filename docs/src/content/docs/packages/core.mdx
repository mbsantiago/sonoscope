...
Every viewer supports calling `attachNavigation(options?)` directly without needing to pass the canvas element or import standalone helpers:

```typescript
// 1. Spectrogram (pan/zoom time and frequency)
const detachSpec = spectrogram.attachNavigation();

// 2. Waveform (defaults to time-only axis)
const detachWave = waveform.attachNavigation({ axis: "time" });
...
You can also attach coordinated navigation directly to any `<canvas>` element from the root `Sonoscope` instance:

```typescript
// Attach navigation for any canvas bound to the scope's timeline
