...
describe("WebGL2 shaders", () => {
  it("compile in a real browser WebGL2 context", () => {
    ...
    expect(
      compileShader(gl, gl.VERTEX_SHADER, WEBGL2_VERTEX_SHADER),
    ).toBeUndefined();
    expect(
  ...
  it("does not stretch the lowest frequency bin up the viewport", () => {
    ...
    Object.defineProperty(canvas2d, "getBoundingClientRect", {
      value: () => ({ width: 16, height: 16 }),
    });
    ...
    expect(
      rowBrightness(webglPixels, canvas.width, webglRow),
    ).toBeLessThanOrEqual(
      rowBrightness(canvasPixels, canvas2d.width, canvasRow) + 2,
    );
  });
...
function brightBandTile(timeStart = 0, timeEnd = 1): SpectrogramMatrix {
  ...
  return {
    ...
    times: Float32Array.from(
      { length: frameCount },
      (_, index) =>
        timeStart + (index / (frameCount - 1)) * (timeEnd - timeStart),
    ),
...
function singleBrightBinTile(brightBin: number): SpectrogramMatrix {
  ...
  const magnitude = new Float32Array(frameCount * binCount);
  for (let frame = 0; frame < frameCount; frame++)
    magnitude[frame * binCount + brightBin] = 1;
  return {
    channel: 0,
    timeStart: 0,
    timeEnd: 1,
...
function rowBrightness(
  ...
  for (let x = 0; x < width; x++)
    total += pixels[(yFromBottom * width + x) * 4]!;
  return total / width;
}

function meanRgbDifference(
  webglPixels: Uint8Array | Uint8ClampedArray,
  canvasPixels: Uint8Array | Uint8ClampedArray,
  width: number,
  height: number,
): number {
  let total = 0;
  ...
  for (let yFromBottom = 0; yFromBottom < height; yFromBottom++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let channel = 0; channel < 3; channel++) {
        total += Math.abs(
          webglPixels[webglIndex + channel]! -
            canvasPixels[canvasIndex + channel]!,
        );
        count += 1;
      }
    }
