...
async function capture(page, renderer) {
  ...
  const canvasPath = new URL(`renderers-${renderer}-canvas.png`, outputDir).pathname;
  ...
  if (stats.brightPixels < 1000) throw new Error(`${renderer} screenshot has too few bright pixels: ${stats.brightPixels}`);
...
function pngStats(buffer) {
  ...
  for (let index = 0; index < png.data.length; index += 4) {
    if (png.data[index] > 64 || png.data[index + 1] > 64 || png.data[index + 2] > 64) brightPixels += 1;
  }
...
function pngDiff(leftBuffer, rightBuffer) {
  ...
  if (left.width !== right.width || left.height !== right.height) throw new Error(`Cannot diff PNGs with different sizes: ${left.width}x${left.height} vs ${right.width}x${right.height}`);
  let total = 0;
  let max = 0;
  let compared = 0;
  ...
  return { meanAbsoluteChannelDifference: total / compared, maxChannelDifference: max };
}
