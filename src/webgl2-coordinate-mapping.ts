...
export function frequencyToTextureV(input: {
  ...
  return Math.max(0, Math.min(1, (input.frequency - input.minFrequency) / span));
}
...
export function viewportPixelToTime(input: { x: number; width: number; viewportStartTime: number; viewportEndTime: number }): number {
  return input.viewportStartTime + (input.x / input.width) * (input.viewportEndTime - input.viewportStartTime);
}

export function timeToFrame(input: { time: number; tileStartTime: number; tileEndTime: number; frameCount: number }): number {
  return Math.max(0, Math.min(input.frameCount - 1, Math.floor(timeToTextureU(input) * input.frameCount)));
}
...
export function viewportPixelToTileSample(input: {
  ...
  frameCount: number;
  binCount: number;
}): { time: number; frequency: number; textureU: number; textureV: number; frame: number; bin: number } {
  const time = viewportPixelToTime({ x: input.x, width: input.width, viewportStartTime: input.viewportStartTime, viewportEndTime: input.viewportEndTime });
  const frequency = viewportPixelToFrequency({ y: input.y, height: input.height, minFrequency: input.viewportMinFrequency, maxFrequency: input.viewportMaxFrequency, frequencyScale: input.frequencyScale });
  const textureU = timeToTextureU({ time, tileStartTime: input.tileStartTime, tileEndTime: input.tileEndTime });
  const textureV = frequencyToWebGLTextureV({ frequency, minFrequency: input.tileMinFrequency, maxFrequency: input.tileMaxFrequency });
  return {
    time,
    frequency,
    textureU,
