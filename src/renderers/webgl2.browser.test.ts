...
import { WEBGL2_DITHER_FRAGMENT_SHADER, WEBGL2_FRAGMENT_SHADER, WEBGL2_SOBEL_FRAGMENT_SHADER, WEBGL2_TERRAIN_FRAGMENT_SHADER, WEBGL2_TERRAIN_VERTEX_SHADER, WEBGL2_VERTEX_SHADER, WebGL2SpectrogramRenderer } from './webgl2';
...
describe('WebGL2 shaders', () => {
  it('compile in a real browser WebGL2 context', () => {
    ...
    expect(compileShader(gl, gl.FRAGMENT_SHADER, WEBGL2_DITHER_FRAGMENT_SHADER)).toBeUndefined();
    expect(compileShader(gl, gl.FRAGMENT_SHADER, WEBGL2_SOBEL_FRAGMENT_SHADER)).toBeUndefined();
  ...
  it('renders visible sobel edge pixels', () => {
    ...
    renderer.render({
      canvas,
      viewport: { startTime: 0, endTime: 1, minFrequency: 0, maxFrequency: 100, frequencyScale: 'linear' },
      valueScale: { mode: 'magnitude', min: -100, max: 0, gamma: 1, clamp: true },
      colorMap: 'gray',
      tiles: [brightBandTile()],
      webglProgram: 'sobel',
    });
