...
export function parseWavHeader(bytes: Uint8Array): WavInfo {
  ...
  if (channelCount <= 0 || sampleRate <= 0 || blockAlign <= 0) throw new Error('Invalid WAV format metadata');
...
export function decodeWavPcm(bytes: Uint8Array, info: WavInfo, byteOffset = info.dataOffset): Float32Array[] {
  ...
  const available = bytes.slice(skipBytes, skipBytes + Math.floor(frameBytes / info.blockAlign) * info.blockAlign);
