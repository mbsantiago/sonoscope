...
describe('createAudioSourceFromUrl', () => {
  it('uses StreamingWavSource for WAV URLs', async () => {
    const streaming = vi.spyOn(StreamingWavSource, 'fromByteSource').mockResolvedValue({
      id: 'streaming',
      sampleRate: 10,
      duration: 1,
      channelCount: 1,
    ...
    globalThis.fetch = vi.fn().mockResolvedValue({ ok: true, body: streamFrom(new Uint8Array(wavHeader(10))) }) as typeof fetch;
  ...
  it('falls back to DecodedAudioSource for unknown URLs and passes options through', async () => {
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce({ ok: true, body: streamFrom(Uint8Array.from([1, 2, 3, 4])) })
      .mockResolvedValueOnce({ ok: true, arrayBuffer: () => Promise.resolve(new ArrayBuffer(0)) }) as typeof fetch;
  ...
  it('falls back to DecodedAudioSource when WAV streaming fails', async () => {
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce({ ok: true, body: streamFrom(new Uint8Array(wavHeader(10))) })
      .mockResolvedValueOnce({ ok: true, arrayBuffer: () => Promise.resolve(wavHeader(10)) }) as typeof fetch;
