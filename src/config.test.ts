...
describe("resolveConfig", () => {
	...
	it("preserves hidden superpower flags", () => {
		expect(
			...
			}).superpowers.secretSpectrogram3d,
		).toBe(true);
	});
	...
	it("preserves explicit renderer modes", () => {
		...
		expect(
			resolveConfig({
				canvas,
				source,
				renderer: { type: "webgl", program: "sobel" },
			}).renderer,
		).toEqual({ type: "webgl", program: "sobel" });
		expect(
			resolveConfig({
				canvas,
				source,
		...
		).toEqual({ type: "webgl", program: "terrain" });
	});
	...
	it("throws when fftSize is not a power of two", () => {
		expect(() =>
			resolveConfig({ canvas, source, stft: { fftSize: 1000 } }),
	...
	it("clamps viewport duration to configured bounds", () => {
		...
		expect(config.viewportConstraints).toEqual({
