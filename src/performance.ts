...
export class FrameMeter {
  ...
  tick(time: number, workMs = 0): FrameStats | undefined {
    ...
    if (this.frames < this.sampleFrames || this.elapsedMs <= 0) return undefined;
    const stats = { frames: this.frames, elapsedMs: this.elapsedMs, fps: (this.frames / this.elapsedMs) * 1000, minFrameMs: this.minFrameMs, maxFrameMs: this.maxFrameMs, averageFrameMs: this.elapsedMs / this.frames, maxWorkMs: this.maxWorkMs, averageWorkMs: this.workMs / this.frames };
    this.reset();
    this.lastTime = time;
    return stats;
  }
