...
export function valueScaleBounds(config: Required<ValueScaleConfig>): { min: number; max: number } {
  if (config.mode === 'magnitude') return { min: magnitudeFromDb(config.min), max: magnitudeFromDb(config.max) };
  if (config.mode === 'power') return { min: powerFromDb(config.min), max: powerFromDb(config.max) };
...
export function deriveValueArrays(matrix: SpectrogramMatrix): SpectrogramMatrix {
  return {
    ...matrix,
