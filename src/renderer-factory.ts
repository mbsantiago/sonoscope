...
export function createSpectrogramRenderer(canvas: HTMLCanvasElement, mode: RendererMode): SpectrogramRenderer {
  ...
  try {
    const renderer = WebGL2SpectrogramRenderer.create(canvas, webglMode && typeof mode.program === 'object' ? mode.program : undefined);
    if (!renderer) {
      if (mode === 'webgl' || mode === 'webgl2' || webglMode) throw new Error(`WebGL renderer requested but WebGL2 is unavailable: ${WebGL2SpectrogramRenderer.diagnose(canvas) ?? 'unknown reason'}`);
