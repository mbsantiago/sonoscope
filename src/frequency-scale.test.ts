...
describe('frequency-scale', () => {
  ...
  it('maps viewport start, midpoint, and end exactly across the canvas width', () => {
    ...
    expect(timeFrequencyToCanvas(viewport.startTime, viewport.minFrequency, width, height, viewport).x).toBe(0);
    expect(canvasToTimeFrequency(0, 0, width, height, viewport).time).toBe(viewport.startTime);

    expect(timeFrequencyToCanvas(15, viewport.minFrequency, width, height, viewport).x).toBe(160);
    expect(canvasToTimeFrequency(160, 0, width, height, viewport).time).toBe(15);

    expect(timeFrequencyToCanvas(viewport.endTime, viewport.minFrequency, width, height, viewport).x).toBe(320);
    expect(canvasToTimeFrequency(320, 0, width, height, viewport).time).toBe(viewport.endTime);
  });

  it('maps fractional times with a non-zero viewport start without pixel drift', () => {
    const fractionalViewport = { ...viewport, startTime: 12.25, endTime: 13.75 };
    const width = 375;
    const time = 12.625;

    const point = timeFrequencyToCanvas(time, 440.5, width, 140, fractionalViewport);

    expect(point.x).toBeCloseTo(93.75, 12);
    expect(canvasToTimeFrequency(point.x, point.y, width, 140, fractionalViewport).time).toBeCloseTo(time, 12);
  });
